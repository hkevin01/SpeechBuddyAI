...
public sealed class NoteStorageService
{
    ...
    private static async Task EnsureSchemaColumnsAsync(SQLiteAsyncConnection db)
    {
        ...
        if (!existingColumns.Contains("AssignmentScoringFormulaVersion"))
            commands.Add("ALTER TABLE SessionNote ADD COLUMN AssignmentScoringFormulaVersion TEXT NOT NULL DEFAULT '';");
        if (!existingColumns.Contains("SessionCalibrationQualitySummary"))
            commands.Add("ALTER TABLE SessionNote ADD COLUMN SessionCalibrationQualitySummary TEXT NOT NULL DEFAULT '';");
